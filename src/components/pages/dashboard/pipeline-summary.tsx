  ...
  return (
    <div className="flex flex-col gap-5 rounded-xl border border-gray-200 bg-white p-5 shadow-card">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          type="button"
          className="flex shrink-0 items-center gap-1 text-body-sm font-semibold text-brand-500 transition-colors hover:text-brand-600"
        >
          Details
          <IconExternalLink />
        </button>
      ...
      <div className="flex flex-col gap-3">
        {data.map((stage) => (
          <div key={stage.label} className="flex items-center gap-4">
            {/* Label */}
            <span className="w-28 shrink-0 text-right text-body-sm font-medium text-gray-500">
              {stage.label}
            </span>
